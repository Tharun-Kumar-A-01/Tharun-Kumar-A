		...
		>
			<div className="bg-background bg-opacity-85 w-full h-full flex items-center justify-between px-5  md:px-10 lg:px-32 ">
					...
					>
						...
						{isMenuOpen ? (
							...
							<svg
								...
								xmlns="http://www.w3.org/2000/svg"
							>
								<rect width="279" height="31" rx="15.5" fill="white" />
								<rect y="97" width="279" height="31" rx="15.5" fill="white" />
								<rect y="194" width="279" height="31" rx="15.5" fill="white" />
							</svg>
						)}
					</button>
				...
				<div>
					<Link href="mailto://tharunkumar.a.2006@gmail.com">
						<button
							className={` ${jetBrainsMono.className} text-sm py-2 px-3 bg-primary rounded-lg font-bold flex flex-row flex-shrink-0 gap-2 items-center text-black `}
						>
							Contact Me{" "}
						</button>
					</Link>
			...
			</div>
			{
				<div
					...
					} transition-all duration-100 sm:hidden absolute top-14 z-20 left-0 w-3/4 sm:w-1/2 md:w-1/4 items-center justify-center gap-2 h-[calc(100vh-3.0rem)] flex flex-col bg-black`}
				>
					{links.map((link) => {
						return (
							<>
